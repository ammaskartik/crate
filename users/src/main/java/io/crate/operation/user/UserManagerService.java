...
public class UserManagerService implements UserManager, ClusterStateListener {
    ...
    private static User crateUser = new User("crate", true);
    private Set<User> users = ImmutableSet.of(crateUser);
    ...
    public void clusterChanged(ClusterChangedEvent event) {
        ...
        users = getNewCurrentUsers(event.state().metaData().custom(UsersMetaData.TYPE));
    }

    static Set<User> getNewCurrentUsers(@Nullable UsersMetaData metaData) {
        ImmutableSet.Builder<User> usersBuilder = new ImmutableSet.Builder<User>().add(crateUser);
        if (metaData != null) {
