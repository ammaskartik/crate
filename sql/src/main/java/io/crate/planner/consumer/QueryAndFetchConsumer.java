...
public class QueryAndFetchConsumer implements Consumer {
    ...
    private final static class Visitor extends RelationPlanningVisitor {
        ...
        public Plan visitQueriedSelectRelation(QueriedSelectRelation relation, ConsumerContext context) {
            ...
            context.setFetchDecider(parentFetchDecider);
    ...
    private static Plan tryToPropagateFetch(QuerySpec querySpec,
        ...
        boolean appliedOrderByAndLimit = tryApplyOrderAndLimitPreFetch(querySpec, plan, fetchDescription, limits);

        if (appliedWhere && appliedOrderByAndLimit && !context.fetchDecider().finalizeFetch()) {
        ...
        if (appliedOrderByAndLimit) {
            // TODO: should we try to skip evalProjection by checking if there are any functions/column-re-orderings?
